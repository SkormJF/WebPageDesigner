...
**Motion carries the product's personality.** A page whose approved direction is quiet and precise should
not have springy overshoot on its buttons, and one that is deliberately playful should not have motion so
restrained it reads as cold. The design system decides the register; this decides how to execute it well.
