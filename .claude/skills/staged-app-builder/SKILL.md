...
- **A new Supabase project has "Confirm email" ON by default, no SMTP configured.** `signUp` succeeds and creates the user but returns **no session**, and the first login answers `400 {"error_code":"email_not_confirmed"}` — Supabase now names the cause explicitly rather than giving the older generic "invalid credentials", so trust the error. There is no way to deliver the confirmation mail without SMTP, so the account is stranded. If the project already has its own approval gate, disabling "Confirm email" (Authentication → Providers → Email) is usually the right call — confirm with the user first, log the decision. **Settle this in Phase 3.5, not here:** left until deploy time it blocks the end-to-end pass, which signs up a disposable account and immediately needs to log in as it.
