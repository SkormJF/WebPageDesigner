...
apply it. The Builder implements inside its assigned phase/scope; the harness invokes review only at the declared phase boundary. That separation keeps a performance pass from quietly becoming an unreviewed refactor without turning every routine change into a separate agent cycle.
...
- **Do not run Lighthouse on every task or build phase.** It is evidence to gather when performance is the question, not a
